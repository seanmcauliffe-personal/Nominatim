...
class Bbox:
    ...
    def __contains__(self, item):
        ...
        if len(item) == 2:
            return (
                self.coord[0] <= item[0] <= self.coord[2]
        ...
        if len(item) == 4:
            return (
                item[0] >= self.coord[0]
                and item[1] <= self.coord[1]
                and item[2] >= self.coord[2]
                and item[3] <= self.coord[3]
            )
...
def check_for_attributes(obj, attrs, presence="present"):
    ...
    for attr in attrs.split(","):
        ...
        if presence == "absent":
            ...
            assert attr in obj, f"No attribute '{attr}'. Full response:\n{_dump_json()}"
