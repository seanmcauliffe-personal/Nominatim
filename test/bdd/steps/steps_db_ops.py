...
def add_data_to_planet_relations(context):
    ...
    with context.db.cursor() as cur:
        cur.execute(
            "SELECT value FROM osm2pgsql_properties WHERE property = 'db_format'"
        )
        row = cur.fetchone()
        if row is None or row["value"] == "1":
            for r in context.table:
                ...
                tags = chain.from_iterable(
                    [(h[5:], r[h]) for h in r.headings if h.startswith("tags+")]
                )

                cur.execute(
                    """INSERT INTO planet_osm_rels (id, way_off, rel_off, parts, members, tags)
                               VALUES (%s, %s, %s, %s, %s, %s)""",
            ...
            for r in context.table:
                if r["members"]:
                    ...
                    for m in r["members"].split(","):
                        mid = NominatimID(m)
                        members.append(
                            {"ref": mid.oid, "role": mid.cls or "", "type": mid.typ}
                        )
                else:
                    members = []
                ...
                cur.execute(
                    """INSERT INTO planet_osm_rels (id, tags, members)
                               VALUES (%s, %s, %s)""",
                    (
                        r["id"],
                        psycopg.types.json.Json(tags),
...
def add_data_to_planet_ways(context):
    ...
    with context.db.cursor() as cur:
        ...
        for r in context.table:
            ...
            cur.execute(
                "INSERT INTO planet_osm_ways (id, nodes, tags) VALUES (%s, %s, %s)",
                (r["id"], nodes, tags),
            )
...
def import_and_index_data_from_place_table(context):
    ...
    context.nominatim.run_nominatim(
        "import", "--continue", "load-data", "--index-noanalyse", "-q", "--offline"
    )
...
def check_place_contents(context, table, exact):
    ...
    with context.db.cursor() as cur:
        ...
        if exact:
            cur.execute(
                pysql.SQL("SELECT osm_type, osm_id, class from")
...
def check_place_has_entry(context, table, oid):
    ...
    with context.db.cursor() as cur:
        ...
        assert cur.rowcount == 0, "Found {} entries for ID {}".format(cur.rowcount, oid)
    ...
    with tokenizer.name_analyzer() as analyzer:
        with context.db.cursor() as cur:
            for row in context.table:
                nid = NominatimID(row["object"])
                nid.row_by_place_id(
                    cur, "search_name", ["ST_X(centroid) as cx", "ST_Y(centroid) as cy"]
                )
                assert cur.rowcount > 0, "No rows found for " + row["object"]
                        ...
                        if name in ("name_vector", "nameaddress_vector"):
                            ...
                            if not exclude:
                                assert len(tokens) >= len(
                                    items
                                ), "No word entry found for {}. Entries found: {!s}".format(
                                    value, len(tokens)
                                )
...
def check_location_postcode(context):
    ...
    with context.db.cursor() as cur:
        ...
        assert cur.rowcount == len(
            list(context.table)
        ), "Postcode table has {} rows, expected {}.".format(
...
def check_word_table_for_postcodes(context, exclude, postcodes):
    ...
    with context.db.cursor() as cur:
        cur.execute(
            "SELECT word FROM word WHERE type = 'P' and word = any(%s)", (plist,)
        )
...
def check_place_addressline(context):
    ...
    with context.db.cursor() as cur:
        for row in context.table:
            ...
            assert cur.rowcount > 0, "No rows found for place %s and address %s" % (
                row["object"],
                row["address"],
            )
    ...
    with context.db.cursor() as cur:
        for row in context.table:
            ...
            if apid is not None:
                ...
                assert cur.rowcount == 0, "Row found for place %s and address %s" % (
                    row["object"],
                    row["address"],
                )
...
def check_location_property_osmline(context, oid, neg):
    ...
    with context.db.cursor() as cur:
        cur.execute(
            """SELECT *, ST_AsText(linegeo) as geomtxt
        ...
        assert (
            not todo
        ), f"Unmatched lines in table: {list(context.table[i] for i in todo)}"
