...
class NominatimEnvironment:
    ...
    def __init__(self, config):
        ...
        self.api_engine = getattr(
            self, f"create_api_request_func_{config['API_ENGINE']}"
        )()
    ...
    def db_drop_database(self, name):
        ...
        with self.connect_database("postgres") as conn:
            ...
            conn.execute(pysql.SQL("DROP DATABASE IF EXISTS") + pysql.Identifier(name))
        ...
        if not self.api_db_done:
            ...
            if not self._reuse_or_drop_db(self.api_test_db):
                ...
                try:
                    ...
                    self.run_nominatim(
                        "add-data", "--tiger-data", str(testdata / "tiger")
                    )
    ...
    def setup_db(self, context):
        ...
        with self.connect_database(self.template_db) as conn:
            conn.autocommit = True
            conn.execute(
                pysql.SQL("DROP DATABASE IF EXISTS") + pysql.Identifier(self.test_db)
            )
            conn.execute(
                pysql.SQL("CREATE DATABASE {} TEMPLATE = {}").format(
                    pysql.Identifier(self.test_db), pysql.Identifier(self.template_db)
                )
            )
    ...
    def _reuse_or_drop_db(self, name):
        ...
        if self.reuse_template:
            with self.connect_database("postgres") as conn:
                num = execute_scalar(
                    conn, "select count(*) from pg_database where datname = %s", (name,)
                )
                if num == 1:
                    return True
        else:
    ...
    def run_nominatim(self, *cmdline):
        ...
        cli.nominatim(osm2pgsql_path=None, cli_args=cmdline, environ=self.test_env)
    ...
    def copy_from_place(self, db):
        ...
        with db.cursor() as cur:
            cur.execute(
                """INSERT INTO placex (osm_type, osm_id, class, type,
    ...
    def create_api_request_func_starlette(self):
        ...
        async def _request(endpoint, params, project_dir, environ, http_headers):
            app = nominatim_api.server.starlette.server.get_application(
                project_dir, environ
            )

            async with LifespanManager(app):
                async with httpx.AsyncClient(
                    app=app, base_url="http://nominatim.test"
                ) as client:
                    response = await client.get(
                        f"/{endpoint}", params=params, headers=http_headers
                    )
    ...
    def create_api_request_func_falcon(self):
        ...
        async def _request(endpoint, params, project_dir, environ, http_headers):
            app = nominatim_api.server.falcon.server.get_application(
                project_dir, environ
            )
