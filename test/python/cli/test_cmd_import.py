...
class TestCliImportWithDb:
    ...
    def test_import_full(
        ...
        mocks = [
            ...
            mock_func_factory(
                nominatim_db.tools.refresh, "import_secondary_importance"
            ),
            mock_func_factory(
                nominatim_db.tools.database_import, "truncate_data_tables"
            ),
            ...
            mock_func_factory(
                nominatim_db.tools.database_import, "create_table_triggers"
            ),
            mock_func_factory(
                nominatim_db.tools.database_import, "create_partition_tables"
            ),
            async_mock_func_factory(
                nominatim_db.tools.database_import, "create_search_indices"
            ),
            mock_func_factory(nominatim_db.data.country_info, "create_country_names"),
            mock_func_factory(
                nominatim_db.tools.refresh, "load_address_levels_from_config"
            ),
        ...
        if with_updates:
            mocks.append(
                mock_func_factory(nominatim_db.tools.freeze, "drop_update_tables")
            )
    ...
    def test_import_continue_load_data(
        self, mock_func_factory, async_mock_func_factory
    ):
        mocks = [
            mock_func_factory(
                nominatim_db.tools.database_import, "truncate_data_tables"
    ...
    def test_import_continue_indexing(
        self, mock_func_factory, async_mock_func_factory, placex_table, temp_db_conn
    ):
        mocks = [
            async_mock_func_factory(
                nominatim_db.tools.database_import, "create_search_indices"
    ...
    def test_import_continue_postprocess(
        self, mock_func_factory, async_mock_func_factory
    ):
        mocks = [
            async_mock_func_factory(
