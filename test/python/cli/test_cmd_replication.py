...
class TestCliReplication:
    ...
    def setup_update_function(self, monkeypatch):
        def _mock_updates(states):
            monkeypatch.setattr(
                nominatim_db.tools.replication,
                "update",
    ...
    def test_replication_update_once_no_index(self, update_mock):
        ...
        assert str(update_mock.last_args[1]["osm2pgsql"]).endswith(
    ...
    def test_replication_catchup(
        self, placex_table, monkeypatch, index_mock, update_interval
    ):
        monkeypatch.setenv(
            "NOMINATIM_REPLICATION_UPDATE_INTERVAL", str(update_interval)
        )
    ...
    def test_replication_update_continuous_no_change(
        self, mock_func_factory, index_mock
    ):
        self.update_states(
            [
