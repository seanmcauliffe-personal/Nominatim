...
class TestCliWithDb:
    ...
    def test_cli_add_data_file_command(self, cli_call, mock_func_factory, name, oid):
        mock_run_legacy = mock_func_factory(
            nominatim_db.tools.add_osm_data, "add_data_from_file"
        )
    ...
    def test_cli_add_data_tiger_data(
        self, cli_call, cli_tokenizer_mock, async_mock_func_factory
    ):
    ...
    def test_freeze_command(self, mock_func_factory):
        ...
        mock_flatnode = mock_func_factory(
            nominatim_db.tools.freeze, "drop_flatnode_file"
        )
    ...
    def test_index_command(
        ...
        bnd_mock = async_mock_func_factory(
            nominatim_db.indexer.indexer.Indexer, "index_boundaries"
        )
        rank_mock = async_mock_func_factory(
            nominatim_db.indexer.indexer.Indexer, "index_by_rank"
        )
        postcode_mock = async_mock_func_factory(
            nominatim_db.indexer.indexer.Indexer, "index_postcodes"
        )

        monkeypatch.setattr(
            nominatim_db.indexer.indexer.Indexer, "has_pending", [False, True].pop
        )
    ...
    def test_special_phrases_wiki_command(self, mock_func_factory):
        func = mock_func_factory(
            nominatim_db.clicmd.special_phrases.SPImporter, "import_phrases"
        )
    ...
    def test_special_phrases_csv_bad_file(self, src_dir):
        ...
        self.call_nominatim(
            "special-phrases", "--import-from-csv", str(testdata.resolve())
        ) == 1
