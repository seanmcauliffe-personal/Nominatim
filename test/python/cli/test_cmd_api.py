...
class TestCliStatusCall:
    ...
    def setup_status_mock(self, monkeypatch):
        monkeypatch.setattr(
            napi.NominatimAPI, "status", lambda self: napi.StatusResult(200, "OK")
        )
    ...
    def test_status_json_format(self, cli_call, tmp_path, capsys):
        result = cli_call("status", "--project-dir", str(tmp_path), "--format", "json")
...
class TestCliDetailsCall:
    ...
    def setup_status_mock(self, monkeypatch):
        result = napi.DetailedResult(
            napi.SourceTable.PLACEX, ("place", "thing"), napi.Point(1.0, -3.0)
        )

        monkeypatch.setattr(
            napi.NominatimAPI, "details", lambda *args, **kwargs: result
        )

    @pytest.mark.parametrize(
        "params",
        [("--node", "1"), ("--way", "1"), ("--relation", "1"), ("--place_id", "10001")],
    )
...
class TestCliReverseCall:
    ...
    def setup_reverse_mock(self, monkeypatch):
        ...
        monkeypatch.setattr(
            napi.NominatimAPI, "reverse", lambda *args, **kwargs: result
        )

    def test_reverse_simple(self, cli_call, tmp_path, capsys):
        result = cli_call(
            "reverse", "--project-dir", str(tmp_path), "--lat", "34", "--lon", "34"
        )
...
class TestCliLookupCall:
    ...
    def test_lookup_simple(self, cli_call, tmp_path, capsys):
        result = cli_call("lookup", "--project-dir", str(tmp_path), "--id", "N34")
...
@pytest.mark.parametrize(
    "endpoint, params",
    [("search", ("--query", "Berlin")), ("search_address", ("--city", "Berlin"))],
)
