    ...
    def _mk_sql(sql_body):
        (tmp_path / "test.sql").write_text(
          ...
          BEGIN
            {}
          END;
          $$ LANGUAGE plpgsql IMMUTABLE;""".format(
                sql_body
            )
        )
        return "test.sql"
...
def test_load_file_simple(
    ...
    SQLPreprocessor(temp_db_conn, sql_preprocessor_cfg).run_sql_file(
        temp_db_conn, sqlfile
    )

    assert temp_db_cursor.scalar("SELECT test()") == ret


def test_load_file_with_params(
    sql_preprocessor, sql_factory, temp_db_conn, temp_db_cursor
):
