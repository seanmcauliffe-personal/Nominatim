...
@pytest.mark.parametrize("invals,outstr", [((1,), "[1]"), (("a", "b"), '["a","b"]')])
...
def test_object_single_entry():
    writer = JsonWriter().start_object().key("something").value(5).end_object()
...
def test_object_many_values_without_none():
    writer = (
        ...
        .keyval_not_none("baz", "")
        .keyval_not_none("eve", False, transform=lambda v: "yes" if v else "no")
        .end_object()
    )
