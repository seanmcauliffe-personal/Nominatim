...
def test_format_reverse_minimal(fmt):
    reverse = napi.ReverseResult(
        napi.SourceTable.PLACEX, ("amenity", "post_box"), napi.Point(0.3, -8.9)
    )
...
def test_format_reverse_with_address(fmt):
    ...
    raw = v1_format.format_result(
        napi.ReverseResults([reverse]), fmt, {"addressdetails": True}
    )
...
def test_format_reverse_geocodejson_special_parts():
    ...
    raw = v1_format.format_result(
        napi.ReverseResults([reverse]), "geocodejson", {"addressdetails": True}
    )
...
def test_format_reverse_with_extratags(fmt):
    ...
    raw = v1_format.format_result(
        napi.ReverseResults([reverse]), fmt, {"extratags": True}
    )
...
def test_format_reverse_with_extratags_none(fmt):
    reverse = napi.ReverseResult(
        napi.SourceTable.PLACEX, ("place", "thing"), napi.Point(1.0, 2.0)
    )
...
def test_format_reverse_with_namedetails_with_name(fmt):
    ...
    raw = v1_format.format_result(
        napi.ReverseResults([reverse]), fmt, {"namedetails": True}
    )
...
def test_search_details_with_icon_available(fmt):
    reverse = napi.ReverseResult(
        napi.SourceTable.PLACEX, ("amenity", "restaurant"), napi.Point(1.0, 2.0)
    )

    result = v1_format.format_result(
        napi.ReverseResults([reverse]), fmt, {"icon_base_url": "foo"}
    )
...
def test_search_details_with_icon_not_available(fmt):
    reverse = napi.ReverseResult(
        napi.SourceTable.PLACEX, ("amenity", "tree"), napi.Point(1.0, 2.0)
    )
