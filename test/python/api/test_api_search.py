...
def test_search_simple_word(apiobj, frontend):
    apiobj.add_word_table(
        [(55, "test", "W", "test", None), (2, "test", "w", "test", None)]
    )

    apiobj.add_placex(place_id=444, class_="place", type="village", centroid=(1.3, 0.7))
...
@pytest.mark.parametrize(
    "atype,address,search",
    [("street", 26, 26), ("city", 16, 18), ("county", 12, 12), ("state", 8, 8)],
)
def test_address_simple_places(apiobj, frontend, atype, address, search):
    ...
    apiobj.add_placex(
        place_id=444, rank_address=address, rank_search=search, centroid=(1.3, 0.7)
    )
...
def test_category_simple_restaurant(apiobj, frontend):
    ...
    apiobj.add_placex(
        place_id=444, class_="amenity", type="restaurant", centroid=(1.3, 0.7)
    )
    apiobj.add_search_name(444, names=[2, 55], address_rank=16, search_rank=18)

    api = frontend(apiobj, options=API_OPTIONS)
    results = api.search_category(
        [("amenity", "restaurant")], near=(1.3, 0.701), near_radius=0.015
    )
...
def test_category_with_search_phrase(apiobj, frontend):
    ...
    apiobj.add_search_name(444, names=[2, 55], address_rank=16, search_rank=18)
    apiobj.add_placex(
        place_id=95, class_="amenity", type="restaurant", centroid=(1.3, 0.7003)
    )
