...
def test_find_near(apiobj, frontend, coord, numres):
    ...
    results = run_search(
        ...
        0.0,
        ["ro"],
        details=SearchDetails(near=napi.Point(*coord), near_radius=0.1),
    )
...
class TestCountryParameters:
    ...
    def test_return_geometries(self, apiobj, frontend, geom, cc):
        results = run_search(
            apiobj, frontend, 0.5, [cc], details=SearchDetails(geometry_output=geom)
        )
    ...
    def test_exclude_place_id(self, apiobj, frontend, pid, rids):
        results = run_search(
            apiobj, frontend, 0.5, ["yw", "ro"], details=SearchDetails(excluded=[pid])
        )

        assert [r.place_id for r in results] == rids

    @pytest.mark.parametrize(
        "viewbox,rids", [((9, 9, 11, 11), [55]), ((-10, -10, -3, -3), [])]
    )
    ...
    @pytest.mark.parametrize(
        "viewbox,numres", [((0, 0, 1, 1), 1), ((-10, -10, -3, -3), 0)]
    )
