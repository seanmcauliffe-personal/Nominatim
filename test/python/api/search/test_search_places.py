...
from nominatim_api.search.db_search_fields import (
    WeightedStrings,
    WeightedCategories,
    FieldLookup,
    FieldRanking,
...
class TestNameOnlySearches:
    ...
    def fill_database(self, apiobj):
        apiobj.add_placex(place_id=100, country_code="us", centroid=(5.6, 4.3))
        apiobj.add_search_name(
            100, names=[1, 2, 10, 11], country_code="us", centroid=(5.6, 4.3)
        )
        apiobj.add_placex(place_id=101, country_code="mx", centroid=(-10.3, 56.9))
        apiobj.add_search_name(
            101, names=[1, 2, 20, 21], country_code="mx", centroid=(-10.3, 56.9)
        )

    @pytest.mark.parametrize("lookup_type", [LookupAll, Restrict])
    @pytest.mark.parametrize("rank,res", [([10], [100, 101]), ([20], [101, 100])])
    ...
    def test_return_simplified_geometry(self, apiobj, frontend, factor, npoints):
        ...
        apiobj.add_search_name(333, names=[55], country_code="us", centroid=(5.6, 4.3))
    ...
    def test_force_viewbox(self, apiobj, frontend, viewbox):
        ...
        details = SearchDetails.from_kwargs(
            {"viewbox": viewbox, "bounded_viewbox": True}
        )
    ...
    def test_force_near(self, apiobj, frontend, radius):
        ...
        details = SearchDetails.from_kwargs({"near": "5.6,4.3", "near_radius": radius})
...
class TestStreetWithHousenumber:
    ...
    def test_lookup_with_country_restriction(self, apiobj, frontend, cc, res):
        ...
        results = run_search(
            apiobj, frontend, 0.1, [lookup], [ranking], hnrs=["22"], ccodes=[cc]
        )
    ...
    def test_lookup_min_rank(self, apiobj, frontend, rank, found):
        ...
        assert [r.place_id for r in results] == (
            [2, 92, 1000, 2000] if found else [2, 92]
        )
...
def test_very_large_housenumber(apiobj, frontend):
    ...
    apiobj.add_search_name(
        2000, names=[1, 2], search_rank=26, address_rank=26, country_code="pt"
    )
...
def test_name_and_postcode(apiobj, frontend, wcount, rids):
    ...
    apiobj.add_search_name(
        990, names=[111], centroid=(10.0, 10.0), search_rank=27, address_rank=27
    )
    apiobj.add_placex(
        place_id=991,
        class_="highway",
    ...
    apiobj.add_search_name(
        991, names=[111], centroid=(10.3, 10.3), search_rank=27, address_rank=27
    )
    apiobj.add_postcode(
...
class TestInterpolations:
    ...
    def fill_database(self, apiobj):
        ...
        apiobj.add_search_name(990, names=[111], search_rank=27, address_rank=27)
        apiobj.add_placex(
            place_id=991,
            class_="place",
            type="house",
...
class TestTiger:
    ...
    def fill_database(self, apiobj):
        ...
        apiobj.add_search_name(
            990, names=[111], country_code="us", search_rank=27, address_rank=27
        )
        apiobj.add_placex(
            place_id=991,
            class_="place",
...
class TestLayersRank30:
    ...
    def fill_database(self, apiobj):
        ...
        apiobj.add_search_name(
            223, names=[34], importance=0.0009, address_rank=30, search_rank=30
        )
        apiobj.add_placex(
            place_id=224,
            class_="amenity",
        ...
        apiobj.add_search_name(
            224, names=[34], importance=0.0008, address_rank=30, search_rank=30
        )
        apiobj.add_placex(
            place_id=225,
            class_="man_made",
        ...
        apiobj.add_search_name(
            225, names=[34], importance=0.0007, address_rank=0, search_rank=30
        )
        apiobj.add_placex(
            place_id=226,
            class_="railway",
        ...
        apiobj.add_search_name(
            226, names=[34], importance=0.0006, address_rank=0, search_rank=30
        )
        apiobj.add_placex(
            place_id=227, class_="natural", type="cave", rank_address=0, rank_search=30
        )
        apiobj.add_search_name(
            227, names=[34], importance=0.0005, address_rank=0, search_rank=30
        )
    ...
    def test_layers_rank30(self, apiobj, frontend, layer, res):
        ...
        results = run_search(
            apiobj, frontend, 0.1, [lookup], [], details=SearchDetails(layers=layer)
        )
