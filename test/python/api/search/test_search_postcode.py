...
def test_postcode_with_country(apiobj, frontend):
    ...
    results = run_search(
        apiobj, frontend, 0.3, ["12345", "12 345"], [0.0, 0.1], ccodes=["de", "pl"]
    )
...
class TestPostcodeSearchWithAddress:
    ...
    def test_lookup_both(self, apiobj, frontend):
        ...
        results = run_search(
            apiobj, frontend, 0.1, ["12345"], lookup=[lookup], ranking=[ranking]
        )
    ...
    @pytest.mark.parametrize(
        "coord,place_id", [((16.5, 5), 100), ((-45.1, 7.004), 101)]
    )
    def test_lookup_near(self, apiobj, frontend, coord, place_id):
        ...
        results = run_search(
            ...
            lookup=[lookup],
            ranking=[ranking],
            details=SearchDetails(near=napi.Point(*coord), near_radius=0.6),
        )
    ...
    @pytest.mark.parametrize(
        "viewbox, rids", [("-46,6,-44,8", [101, 100]), ("16,4,18,6", [100, 101])]
    )
    ...
    @pytest.mark.parametrize("viewbox, rid", [("-46,6,-44,8", 101), ("16,4,18,6", 100)])
    ...
    @pytest.mark.parametrize(
        "coord,rids", [((17.05, 5), [100, 101]), ((-45, 7.1), [101, 100])]
    )
    ...
    def test_exclude(self, apiobj, frontend, pid, rid):
        results = run_search(
            apiobj, frontend, 0.1, ["12345"], details=SearchDetails(excluded=[pid])
        )
