        ...
        for end, ttype, tinfo in tlist:
            for tid, word in tinfo:
                q.add_token(
                    ...
                    ttype,
                    MyToken(
                        penalty=0.5 if ttype == TokenType.PARTIAL else 0.0,
                        token=tid,
                        count=1,
                        addr_count=1,
                        lookup_word=word,
...
def test_postcode_with_country():
    q = make_query(
        [(1, TokenType.POSTCODE, [(34, "2367")])], [(2, TokenType.COUNTRY, [(1, "xx")])]
    )
    builder = SearchBuilder(q, SearchDetails())

    searches = list(
        builder.build(
            TokenAssignment(postcode=TokenRange(0, 1), country=TokenRange(1, 2))
        )
    )
...
def test_postcode_with_address_with_full_word():
    q = make_query(
        [(1, TokenType.POSTCODE, [(34, "2367")])],
        [(2, TokenType.PARTIAL, [(100, "word")]), (2, TokenType.WORD, [(1, "full")])],
    )
    builder = SearchBuilder(q, SearchDetails())
...
@pytest.mark.parametrize(
    "kwargs", [{"viewbox": "0,0,1,1", "bounded_viewbox": True}, {"near": "10,10"}]
)
...
def test_name_only_search():
    q = make_query(
        [(1, TokenType.PARTIAL, [(1, "a")]), (1, TokenType.WORD, [(100, "a")])]
    )
    builder = SearchBuilder(q, SearchDetails())
...
def test_name_with_qualifier():
    ...
    searches = list(
        builder.build(
            TokenAssignment(name=TokenRange(0, 1), qualifier=TokenRange(1, 2))
        )
    )
...
def test_name_and_address():
    q = make_query(
        [(1, TokenType.PARTIAL, [(1, "a")]), (1, TokenType.WORD, [(100, "a")])],
        [(2, TokenType.PARTIAL, [(2, "b")]), (2, TokenType.WORD, [(101, "b")])],
        [(3, TokenType.PARTIAL, [(3, "c")]), (3, TokenType.WORD, [(102, "c")])],
    )
    builder = SearchBuilder(q, SearchDetails())
    ...
    searches = list(
        builder.build(
            TokenAssignment(
                name=TokenRange(0, 1), address=[TokenRange(1, 2), TokenRange(2, 3)]
            )
        )
    )
...
def test_name_and_complex_address():
    q = make_query(
        [(1, TokenType.PARTIAL, [(1, "a")]), (1, TokenType.WORD, [(100, "a")])],
        [(2, TokenType.PARTIAL, [(2, "b")]), (3, TokenType.WORD, [(101, "bc")])],
        [(3, TokenType.PARTIAL, [(3, "c")])],
        [(4, TokenType.PARTIAL, [(4, "d")]), (4, TokenType.WORD, [(103, "d")])],
    )
    builder = SearchBuilder(q, SearchDetails())
    ...
    searches = list(
        builder.build(
            TokenAssignment(
                name=TokenRange(0, 1), address=[TokenRange(1, 2), TokenRange(2, 4)]
            )
        )
    )
...
def test_name_only_near_search():
    q = make_query(
        [(1, TokenType.NEAR_ITEM, [(88, "g")])],
        [(2, TokenType.PARTIAL, [(1, "a")]), (2, TokenType.WORD, [(100, "a")])],
    )
    builder = SearchBuilder(q, SearchDetails())

    searches = list(
        builder.build(
            TokenAssignment(name=TokenRange(1, 2), near_item=TokenRange(0, 1))
        )
    )
...
def test_name_only_search_with_category():
    ...
    builder = SearchBuilder(
        q, SearchDetails.from_kwargs({"categories": [("foo", "bar")]})
    )
...
def test_name_with_near_item_search_with_category_match():
    ...
    builder = SearchBuilder(
        q, SearchDetails.from_kwargs({"categories": [("foo", "bar"), ("this", "that")]})
    )
...
def test_name_with_qualifier_search_with_category_mismatch():
    ...
    searches = list(
        builder.build(
            TokenAssignment(name=TokenRange(1, 2), qualifier=TokenRange(0, 1))
        )
    )
...
def make_counted_searches(
    name_part, name_full, address_part, address_full, num_address_parts=1
):
    q = QueryStruct([Phrase(PhraseType.NONE, "")])
    ...
    q.add_token(
        TokenRange(0, 1), TokenType.PARTIAL, MyToken(0.5, 1, name_part, 1, "name_part")
    )
    q.add_token(
        TokenRange(0, 1), TokenType.WORD, MyToken(0, 101, name_full, 1, "name_full")
    )
    ...
    return list(
        builder.build(
            TokenAssignment(
                name=TokenRange(0, 1), address=[TokenRange(1, 1 + num_address_parts)]
            )
        )
    )
