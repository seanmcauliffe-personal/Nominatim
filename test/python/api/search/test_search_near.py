...
def test_no_appropriate_results_inner_query(apiobj, frontend):
    ...
    apiobj.add_search_name(100, names=[56], country_code="us", centroid=(5.6, 4.3))
    apiobj.add_placex(
        place_id=22, class_="amenity", type="bank", centroid=(5.6001, 4.2994)
    )
...
class TestNearSearch:
    ...
    def fill_database(self, apiobj):
        ...
        apiobj.add_search_name(
            101, names=[56], country_code="mx", centroid=(-10.3, 56.9)
        )
    ...
    def test_near_in_placex(self, apiobj, frontend):
        ...
        apiobj.add_placex(
            place_id=23, class_="amenity", type="bench", centroid=(5.6001, 4.2994)
        )
    ...
    def test_multiple_types_near_in_placex(self, apiobj, frontend):
        ...
        results = run_search(
            apiobj, frontend, 0.1, [("amenity", "bank"), ("amenity", "bench")]
        )
    ...
    def test_near_in_classtype(self, apiobj, frontend):
        apiobj.add_placex(
            place_id=22, class_="amenity", type="bank", centroid=(5.6, 4.34)
        )
        apiobj.add_placex(
            place_id=23, class_="amenity", type="bench", centroid=(5.6, 4.34)
        )
    ...
    def test_restrict_by_country(self, apiobj, frontend, cc, rid):
        ...
        results = run_search(
            apiobj, frontend, 0.1, [("amenity", "bank")], ccodes=[cc, "fr"]
        )
    ...
    @pytest.mark.parametrize(
        "layer,rids", [(napi.DataLayer.POI, [22]), (napi.DataLayer.MANMADE, [])]
    )
