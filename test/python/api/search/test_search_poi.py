...
@pytest.mark.parametrize("coord,pid", [("34.3, 56.100021", 2), ("5.0, 4.59933", 1)])
def test_simple_near_search_in_placex(apiobj, frontend, coord, pid):
    apiobj.add_placex(
        place_id=1, class_="highway", type="bus_stop", centroid=(5.0, 4.6)
    )
    apiobj.add_placex(
        place_id=2, class_="highway", type="bus_stop", centroid=(34.3, 56.1)
    )

    details = SearchDetails.from_kwargs({"near": coord, "near_radius": 0.001})

    results = run_search(
        apiobj, frontend, 0.1, [("highway", "bus_stop")], [0.5], details=details
    )
...
@pytest.mark.parametrize(
    "coord,pid", [("34.3, 56.100021", 2), ("34.3, 56.4", 2), ("5.0, 4.59933", 1)]
)
