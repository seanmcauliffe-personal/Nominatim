...
def test_status_format_text():
    assert (
        v1_format.format_result(napi.StatusResult(0, "message here"), "text", {})
        == "OK"
    )
...
def test_status_format_text():
    assert (
        v1_format.format_result(napi.StatusResult(500, "message here"), "text", {})
        == "ERROR: message here"
    )
    ...
    setattr(
        ...
        [
            napi.AddressLine(
                ...
                category=("bnd", "note"),
                names={"name": "Trespass"},
                extratags={"access": "no", "place_type": "spec"},
                admin_level=4,
                fromarea=True,
                isaddress=True,
                rank_address=10,
...
def test_search_details_keywords_name():
    ...
    assert js["keywords"] == {
        "name": [{"id": 23, "token": "foo"}, {"id": 24, "token": "foo"}],
        "address": [],
    }
...
def test_search_details_keywords_address():
    ...
    assert js["keywords"] == {
        "address": [{"id": 23, "token": "foo"}, {"id": 24, "token": "foo"}],
        "name": [],
    }
