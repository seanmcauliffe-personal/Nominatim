...
@pytest.mark.parametrize(
    "idobj", (napi.PlaceID(332), napi.OsmID("W", 4), napi.OsmID("W", 4, "highway"))
)
...
def test_lookup_in_placex_with_geometry(apiobj, frontend):
    apiobj.add_placex(place_id=332, geometry="LINESTRING(23 34, 23.1 34)")

    api = frontend(apiobj, options={"details"})
    result = api.details(napi.PlaceID(332), geometry_output=napi.GeometryFormat.GEOJSON)

    assert result.geometry == {
        "geojson": '{"type":"LineString","coordinates":[[23,34],[23.1,34]]}'
    }
...
def test_lookup_in_osmline_split_interpolation(apiobj, frontend):
    apiobj.add_osmline(place_id=1000, osm_id=9, startnumber=2, endnumber=4, step=1)
    apiobj.add_osmline(place_id=1001, osm_id=9, startnumber=6, endnumber=9, step=1)
    apiobj.add_osmline(place_id=1002, osm_id=9, startnumber=11, endnumber=20, step=1)
...
def test_lookup_osmline_with_address_details(apiobj, frontend):
    apiobj.add_osmline(
        place_id=9000, osm_id=9, startnumber=2, endnumber=4, step=1, parent_place_id=332
    )
    apiobj.add_placex(
        place_id=332,
        osm_type="W",
...
def test_lookup_tiger_with_address_details(apiobj, frontend):
    apiobj.add_tiger(
        place_id=9000, startnumber=2, endnumber=4, step=1, parent_place_id=332
    )
    apiobj.add_placex(
        place_id=332,
        osm_type="W",
...
@pytest.mark.parametrize(
    "objid", [napi.PlaceID(1736), napi.OsmID("W", 55), napi.OsmID("N", 55, "amenity")]
)
def test_lookup_missing_object(apiobj, frontend, objid):
    apiobj.add_placex(
        place_id=1, osm_type="N", osm_id=55, class_="place", type="suburb"
    )
...
@pytest.mark.parametrize(
    "gtype",
    (napi.GeometryFormat.KML, napi.GeometryFormat.SVG, napi.GeometryFormat.TEXT),
)
