...
class TestWithDefault:
    ...
    def run_sanitizer_on(self, type, **kwargs):
        ...
        place = PlaceInfo(
            {
                type: {k.replace("_", ":"): v for k, v in kwargs.items()},
                "country_code": "de",
                "rank_address": 30,
            }
        )
        ...
        name, address = PlaceSanitizer([sanitizer_args], self.config).process_names(
            place
        )
...
class TestTypeField:
    ...
    def run_sanitizer_on(self, type, **kwargs):
        ...
        place = PlaceInfo(
            {
                "name": {k.replace("_", ":"): v for k, v in kwargs.items()},
                "country_code": "de",
                "rank_address": 30,
            }
        )
        ...
        name, _ = PlaceSanitizer([sanitizer_args], self.config).process_names(place)
...
class TestFilterKind:
    ...
    def test_single_exact_name(self):
        res = self.run_sanitizer_on(
            ["name"], ref="foo", name="foo", name_abc="bar", ref_abc="bar"
        )
...
class TestRankAddress:
    ...
    def run_sanitizer_on(self, rank_addr, **kwargs):
        ...
        sanitizer_args = {"step": "delete-tags", "rank_address": rank_addr}
    ...
    def test_mixed_rank_pass(self):
        res = self.run_sanitizer_on(
            ["4", "20-28", "30", "10-12"], name="foo", ref="bar"
        )
...
class TestSuffix:
    ...
    def test_single_suffix(self):
        res = self.run_sanitizer_on(
            "abc", name="foo", name_abc="foo", name_pqr="bar", ref="bar", ref_abc="baz"
        )
