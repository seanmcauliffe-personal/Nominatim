...
def test_string_list_value_string():
    assert SanitizerConfig({"op": "t"}).get_string_list("op", default=["a", "b"]) == [
        "t"
    ]
...
def test_create_kind_filter_custom_regex_positive(kind):
    filt = SanitizerConfig({"filter-kind": "lang.*"}).get_filter(
        "filter-kind", [".*fr"]
    )
...
def test_create_kind_filter_many_positive(kind):
    filt = SanitizerConfig({"filter-kind": [".*fr", "name", r"\d+"]}).get_filter(
        "filter-kind"
    )
