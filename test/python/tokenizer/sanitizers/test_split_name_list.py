...
class TestSplitName:
    ...
    def run_sanitizer_on(self, **kwargs):
        ...
        name, _ = PlaceSanitizer(
            [{"step": "split-name-list"}], self.config
        ).process_names(place)
    ...
    def sanitize_with_delimiter(self, delimiter, name):
        ...
        san = PlaceSanitizer(
            [{"step": "split-name-list", "delimiters": delimiter}], self.config
        )
    ...
    def test_splits(self):
        assert self.run_sanitizer_on(name="A;B;C") == [
            ("A", "name", None),
            ("B", "name", None),
            ("C", "name", None),
        ]
        assert self.run_sanitizer_on(short_name=" House, boat ") == [
            ("House", "short_name", None),
            ("boat", "short_name", None),
        ]

    def test_empty_fields(self):
        assert self.run_sanitizer_on(name="A;;B") == [
            ("A", "name", None),
            ("B", "name", None),
        ]
        assert self.run_sanitizer_on(name="A; ,B") == [
...
def test_no_name_list(def_config):
    ...
    name, address = PlaceSanitizer(
        [{"step": "split-name-list"}], def_config
    ).process_names(place)
