...
class TestWithDefaults:
    ...
    def run_sanitizer_on(self, country, **kwargs):
        ...
        name, _ = PlaceSanitizer(
            [{"step": "tag-analyzer-by-language"}], self.config
        ).process_names(place)
...
class TestFilterKind:
    ...
    def run_sanitizer_on(self, filt, **kwargs):
        ...
        name, _ = PlaceSanitizer(
            [{"step": "tag-analyzer-by-language", "filter-kind": filt}], self.config
        ).process_names(place)
    ...
    def test_single_exact_name(self):
        res = self.run_sanitizer_on(
            ["name"], name_fr="A", ref_fr="12", shortname_fr="C", name="D"
        )
...
class TestDefaultCountry:
    ...
    def run_sanitizer_append(self, mode, country, **kwargs):
        ...
        assert all(
            not p.attr or ("analyzer" in p.attr and p.attr["analyzer"]) for p in name
        )
