...
def tokenizer_factory(
    dsn, tmp_path, property_table, sql_preprocessor, place_table, word_table
):
...
@pytest.fixture
def analyzer(
    tokenizer_factory, test_config, monkeypatch, temp_db_with_extensions, tmp_path
):
    ...
    def _mk_analyser(
        ...
        cfgstr = {
            "normalization": list(norm),
            "sanitizers": sanitizers,
            "transliteration": list(trans),
            "token-analysis": [
                {"analyzer": "generic", "variants": [{"words": list(variants)}]}
            ],
        }
        if with_housenumber:
            cfgstr["token-analysis"].append(
                {"id": "@housenumber", "analyzer": "housenumbers"}
            )
        if with_postcode:
            cfgstr["token-analysis"].append(
                {"id": "@postcode", "analyzer": "postcodes"}
            )
...
def getorcreate_full_word(temp_db_cursor):
    temp_db_cursor.execute(
        """CREATE OR REPLACE FUNCTION getorcreate_full_word(
...
def test_init_new(tokenizer_factory, test_config, db_prop):
    ...
    assert db_prop(
        nominatim_db.tokenizer.icu_rule_loader.DBCFG_IMPORT_NORM_RULES
    ).startswith(":: lower ();")
...
def test_update_sql_functions(
    db_prop, temp_db_cursor, tokenizer_factory, test_config, table_factory, monkeypatch
):
    tok = tokenizer_factory()
    tok.init_new_db(test_config)
...
def test_finalize_import(
    tokenizer_factory, temp_db_cursor, test_config, sql_preprocessor_cfg
):
    tok = tokenizer_factory()
    tok.init_new_db(test_config)
...
def test_update_statistics(
    word_table, table_factory, temp_db_cursor, tokenizer_factory, test_config
):
...
class TestPostcodes:
    ...
    def process_postcode(self, cc, postcode):
        return self.analyzer.process_place(
            PlaceInfo({"country_code": cc, "address": {"postcode": postcode}})
        )

    def test_update_postcodes_from_db_empty(self, table_factory, word_table):
        table_factory(
            "location_postcode",
            "country_code TEXT, postcode TEXT",
            content=(
                ("de", "12345"),
                ("se", "132 34"),
                ("bm", "AB23"),
...
class TestPlaceNames:
    ...
    @pytest.fixture(autouse=True)
    def setup(self, analyzer, sql_functions):
        sanitizers = [{"step": "split-name-list"}, {"step": "strip-brace-terms"}]
    ...
    def test_names_with_separator(self, sep):
        ...
        self.expect_name_terms(
            info, "#New York", "#Big Apple", "new", "york", "big", "apple"
        )
    ...
    def test_full_names_with_bracket(self):
        ...
        self.expect_name_terms(
            info, "#Houseboat (left)", "#Houseboat", "houseboat", "left"
        )
...
class TestPlaceAddress:
    ...
    def test_process_place_housenumbers_duplicates(self, getorcreate_hnr_id):
        info = self.process_address(
            housenumber="134", conscriptionnumber="134", streetnumber="99a"
        )
    ...
    def test_process_place_multiple_street_tags(self):
        self.analyzer.process_place(
            PlaceInfo({"name": {"name": "Grand Road", "ref": "05989"}})
        )
        info = self.process_address(
            **{"street": "Grand Road", "street:sym_ul": "05989"}
        )
    ...
    def test_process_place_address_terms_empty(self):
        info = self.process_address(
            country="de", city=" ", street="Hauptstr", full="right behind the church"
        )
...
class TestPlaceHousenumberWithAnalyser:
    ...
    def setup(self, analyzer, sql_functions):
        ...
        with analyzer(
            trans=(":: upper()", "'🜵' > ' '"), sanitizers=[hnr], with_housenumber=True
        ) as anl:
            self.analyzer = anl
            yield anl
...
class TestUpdateWordTokens:
    ...
    def search_entry(self, temp_db_cursor):
        ...
        def _insert(*args):
            temp_db_cursor.execute(
                "INSERT INTO search_name VALUES (%s, %s)", (next(place_id), list(args))
            )
    ...
    def test_keep_housenumbers_from_search_name_table(
        self, add_housenumber, word_table, search_entry
    ):
    ...
    def test_keep_housenumbers_from_placex_table(
        self, add_housenumber, word_table, placex_table
    ):
        add_housenumber(9999, "5432a")
    ...
    def test_keep_housenumbers_from_placex_table_hnr_list(
        self, add_housenumber, word_table, placex_table
    ):
