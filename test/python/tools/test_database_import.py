...
def test_import_osm_data_simple_ignore_no_data(table_factory, osm2pgsql_options):
    ...
    database_import.import_osm_data(
        Path("file.pbf"), osm2pgsql_options, ignore_errors=True
    )


def test_import_osm_data_drop(
    table_factory, temp_db_cursor, tmp_path, osm2pgsql_options
):
    table_factory("place", content=((1,),))
...
def test_truncate_database_tables(
    temp_db_conn, temp_db_cursor, table_factory, with_search
):
    tables = [
        "placex",
        "place_addressline",
...
async def test_load_data(
    dsn, place_row, placex_table, osmline_table, temp_db_cursor, threads
):
    for func in (
        "precompute_words",
        "getorcreate_housenumber_id",
        "make_standard_name",
    ):
        temp_db_cursor.execute(
            pysql.SQL(
                """CREATE FUNCTION {} (src TEXT)
