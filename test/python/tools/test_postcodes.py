...
class MockPostcodeTable:
    ...
    def add(self, country, postcode, x, y):
        with self.conn.cursor() as cur:
            cur.execute(
                """INSERT INTO location_postcode (place_id, indexed_status,
...
def test_postcodes_empty(dsn, postcode_table, place_table, tmp_path, tokenizer):
    postcodes.update_postcodes(dsn, tmp_path, tokenizer)

    assert not postcode_table.row_set


def test_postcodes_add_new(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
    ...
    assert postcode_table.row_set == {
        ("xx", "9486", 10, 12),
    }


def test_postcodes_replace_coordinates(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
...
def test_postcodes_replace_coordinates_close(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
    insert_implicit_postcode(1, "xx", "POINT(10 12)", dict(postcode="AB 4511"))
...
def test_postcodes_ignore_empty_country(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
...
def test_postcodes_remove_all(dsn, postcode_table, place_table, tmp_path, tokenizer):
...
def test_postcodes_multi_country(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
...
def test_postcodes_extern(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer, gzipped
):
    insert_implicit_postcode(1, "xx", "POINT(10 12)", dict(postcode="AB 4511"))
...
def test_postcodes_extern_bad_column(
    dsn, postcode_table, tmp_path, insert_implicit_postcode, tokenizer
):
    insert_implicit_postcode(1, "xx", "POINT(10 12)", dict(postcode="AB 4511"))
...
def test_postcodes_extern_bad_number(
    dsn, insert_implicit_postcode, postcode_table, tmp_path, tokenizer
):
    insert_implicit_postcode(1, "xx", "POINT(10 12)", dict(postcode="AB 4511"))
...
def test_no_placex_entry(
    dsn, tmp_path, temp_db_cursor, place_row, postcode_table, tokenizer
):
...
def test_discard_badly_formatted_postcodes(
    dsn, tmp_path, temp_db_cursor, place_row, postcode_table, tokenizer
):
    # Rewrite the get_country_code function to verify its execution.
    temp_db_cursor.execute(
