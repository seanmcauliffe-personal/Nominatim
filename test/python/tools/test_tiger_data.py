...
class MockTigerTable:
    ...
    def __init__(self, conn):
        ...
        with conn.cursor() as cur:
            cur.execute(
                """CREATE TABLE tiger (linegeo GEOMETRY,
                                               start INTEGER,
                                               stop INTEGER,
                                               interpol TEXT,
                                               token_info JSONB,
...
@pytest.fixture
def tiger_table(
    def_config, temp_db_conn, sql_preprocessor, temp_db_with_extensions, tmp_path
):
...
def csv_factory(tmp_path):
    def _mk_file(
        ...
        (tmp_path / (fname + ".csv")).write_text(
            dedent(
                """\
        from;to;interpolation;street;city;state;postcode;geometry
        {};{};{};{};{};{};{};{}
        """.format(
                    hnr_from, hnr_to, interpol, street, city, state, postcode, geometry
                )
            )
        )
...
@pytest.mark.asyncio
async def test_add_tiger_data(
    def_config, src_dir, tiger_table, tokenizer_mock, threads
):
    await tiger_data.add_tiger_data(
...
@pytest.mark.asyncio
async def test_add_tiger_data_database_frozen(
    def_config, temp_db_conn, tiger_table, tokenizer_mock, tmp_path
):
...
@pytest.mark.asyncio
async def test_add_tiger_data_no_files(
    def_config, tiger_table, tokenizer_mock, tmp_path
):
    await tiger_data.add_tiger_data(str(tmp_path), def_config, 1, tokenizer_mock())
...
@pytest.mark.asyncio
async def test_add_tiger_data_bad_file(
    def_config, tiger_table, tokenizer_mock, tmp_path
):
...
@pytest.mark.asyncio
async def test_add_tiger_data_hnr_nan(
    def_config, tiger_table, tokenizer_mock, csv_factory, tmp_path
):
...
@pytest.mark.parametrize("threads", (1, 5))
@pytest.mark.asyncio
async def test_add_tiger_data_tarfile(
    def_config, tiger_table, tokenizer_mock, tmp_path, src_dir, threads
):
    ...
    await tiger_data.add_tiger_data(
        str(tmp_path / "sample.tar.gz"), def_config, threads, tokenizer_mock()
    )
...
@pytest.mark.asyncio
async def test_add_tiger_data_bad_tarfile(
    def_config, tiger_table, tokenizer_mock, tmp_path
):
...
@pytest.mark.asyncio
async def test_add_tiger_data_empty_tarfile(
    def_config, tiger_table, tokenizer_mock, tmp_path
):
    tar = tarfile.open(str(tmp_path / "sample.tar.gz"), "w:gz")
    tar.add(__file__)
    ...
    await tiger_data.add_tiger_data(
        str(tmp_path / "sample.tar.gz"), def_config, 1, tokenizer_mock()
    )
