...
def test_run_single_migration(
    ...
    oldversion = [4, 4, 99, 0]
    property_table.set(
        "database_version", str(nominatim_db.version.NominatimVersion(*oldversion))
    )
    ...
    assert done["new"]
    assert not done["old"]
    assert property_table.get("database_version") == str(
        nominatim_db.version.NOMINATIM_VERSION
    )
