...
def xml_wiki_content(src_dir):
    ...
    """
    xml_test_content = (
        src_dir / "test" / "testdata" / "special_phrases_test_content.txt"
    )
...
def test_create_place_classtype_indexes(
    temp_db_with_extensions, temp_db_conn, temp_db_cursor, table_factory, sp_importer
):
    """
...
def test_create_place_classtype_table(
    temp_db_conn, temp_db_cursor, placex_table, sp_importer
):
    """
...
def test_grant_access_to_web_user(
    temp_db_conn, temp_db_cursor, table_factory, def_config, sp_importer
):
    """
    ...
    assert check_grant_access(
        temp_db_cursor, def_config.DATABASE_WEBUSER, phrase_class, phrase_type
    )


def test_create_place_classtype_table_and_indexes(
    temp_db_cursor, def_config, placex_table, sp_importer, temp_db_conn
):
    """
    ...
    for pair in pairs:
        ...
        assert check_grant_access(
            temp_db_cursor, def_config.DATABASE_WEBUSER, pair[0], pair[1]
        )


def test_remove_non_existent_tables_from_db(
    sp_importer, default_phrases, temp_db_conn, temp_db_cursor
):
    """
...
def test_import_phrases(
    ...
    monkeypatch.setattr(
        "nominatim_db.tools.special_phrases.sp_wiki_loader._get_wiki_content",
    ...
    assert check_grant_access(
        temp_db_cursor, def_config.DATABASE_WEBUSER, class_test, type_test
    )
...
def check_table_exist(temp_db_cursor, phrase_class, phrase_type):
    ...
    """
    return temp_db_cursor.table_exists(
        "place_classtype_{}_{}".format(phrase_class, phrase_type)
    )
...
def check_grant_access(temp_db_cursor, user, phrase_class, phrase_type):
    ...
    temp_db_cursor.execute(
        """
            ...
            WHERE table_name='{}'
            AND grantee='{}'
            AND privilege_type='SELECT'""".format(
            table_name, user
        )
    )
...
def check_placeid_and_centroid_indexes(temp_db_cursor, phrase_class, phrase_type):
    ...
    return temp_db_cursor.index_exists(
        table_name, index_prefix + "centroid"
    ) and temp_db_cursor.index_exists(table_name, index_prefix + "place_id")
