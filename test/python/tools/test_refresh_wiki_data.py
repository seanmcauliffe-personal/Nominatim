    ...
    def _import(data):
        with gzip.open(tmp_path / "wikimedia-importance.csv.gz", mode="wt") as fd:
            writer = csv.DictWriter(
                fd,
                fieldnames=["language", "type", "title", "importance", "wikidata_id"],
                delimiter="\t",
                quotechar="|",
            )
            writer.writeheader()
...
@pytest.mark.parametrize(
    "extra", [{"wikipedia:en": "Test"}, {"wikipedia": "en:Test"}, {"wikidata": "Q123"}]
)
def test_wikipedia(
    dsn, temp_db_conn, temp_db_cursor, def_config, wiki_csv, placex_table, extra
):
...
def test_wikipedia_no_match(
    dsn, temp_db_conn, temp_db_cursor, def_config, wiki_csv, placex_table
):
