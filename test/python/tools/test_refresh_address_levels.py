...
from nominatim_db.tools.refresh import (
    load_address_levels,
    load_address_levels_from_config,
)
...
def test_load_ranks_country(temp_db_conn, temp_db_cursor):
    load_address_levels(
        ...
        [
            ...
            {"countries": ["de"], "tags": {"place": {"village": 15}}},
            {"countries": ["uk", "us"], "tags": {"place": {"village": 16}}},
        ],
    )

    assert temp_db_cursor.row_set("SELECT * FROM levels") == set(
        [
            (None, "place", "village", 14, 14),
...
def test_load_ranks_default_value(temp_db_conn, temp_db_cursor):
    load_address_levels(
        ...
        [
            {"tags": {"boundary": {"": 28}}},
            {"countries": ["hu"], "tags": {"boundary": {"": 29}}},
        ],
    )

    assert temp_db_cursor.row_set("SELECT * FROM levels") == set(
        [
            (None, "boundary", None, 28, 28),
...
def test_load_ranks_multiple_keys(temp_db_conn, temp_db_cursor):
    load_address_levels(
        temp_db_conn,
        "levels",
        [{"tags": {"place": {"city": 14}, "boundary": {"administrative2": 4}}}],
    )

    assert temp_db_cursor.row_set("SELECT * FROM levels") == set(
        [
            (None, "place", "city", 14, 14),
...
def test_load_ranks_address(temp_db_conn, temp_db_cursor):
    load_address_levels(
        temp_db_conn, "levels", [{"tags": {"place": {"city": 14, "town": [14, 13]}}}]
    )
