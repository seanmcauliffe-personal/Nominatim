...
def test_import_osm_file_simple(dsn, table_factory, osm2pgsql_options, capfd):
    ...
    assert (
        add_osm_data.add_data_from_file(dsn, Path("change.osm"), osm2pgsql_options) == 0
    )
...
@pytest.mark.parametrize(
    "main_api,url",
    [
        (True, "https://www.openstreetmap.org/api"),
        (False, "https://overpass-api.de/api/interpreter?"),
    ],
)
def test_import_osm_object_main_api(
    dsn, osm2pgsql_options, monkeypatch, capfd, osm_type, main_api, url
):
