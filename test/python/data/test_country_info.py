...
def test_setup_country_tables(
    src_dir, temp_db_with_extensions, dsn, temp_db_cursor, loaded_country, no_partitions
):
    country_info.setup_country_tables(dsn, src_dir / "data", no_partitions)

    assert temp_db_cursor.table_exists("country_name")
    assert temp_db_cursor.table_rows("country_name") == temp_db_cursor.scalar(
...
def test_create_country_names(
    temp_db_with_extensions,
    temp_db_conn,
    temp_db_cursor,
    table_factory,
    tokenizer_mock,
    ...
    table_factory(
        "country_name",
        "country_code varchar(2), name hstore",
        content=(
            ("us", '"name"=>"us1","name:af"=>"us2"'),
...
def test_setup_country_names_prefixes(env_with_country_config):
    ...
    assert info.get("es")["names"] == {
        "name": "Espagñe",
        "name:en": "Spain",
        "name:de": "Spanien",
    }
    assert info.get("us")["names"] == {
        "name": "United States",
...
def test_setup_country_config_languages_not_loaded(env_with_country_config):
    ...
    assert dict(info.items()) == {
        "de": {"partition": 3, "languages": [], "names": {"name": "Deutschland"}}
    }
...
def test_setup_country_config_name_not_loaded(env_with_country_config):
    ...
    assert dict(info.items()) == {
        "de": {"partition": 3, "languages": ["de"], "names": {}}
    }
...
def test_setup_country_config_special_character(env_with_country_config):
    ...
    assert dict(info.items()) == {
        "bq": {"partition": 250, "languages": ["nl"], "names": {"name": "\x85"}}
    }
