...
class CommandlineParser:
    ...
    def __init__(self, prog: str, description: Optional[str]):
        ...
        self.subs = self.parser.add_subparsers(
            title="available commands", dest="subcommand"
        )

        # Global arguments that only work if no sub-command given
        self.parser.add_argument(
            "--version", action="store_true", help="Print Nominatim version and exit"
        )
    ...
    def run(self, **kwargs: Any) -> int:
        ...
        args.config = Configuration(
            args.project_dir, environ=kwargs.get("environ", os.environ)
        )
    ...
    async def run_uvicorn(self, args: NominatimArgs) -> None:
        ...
        if len(server_info) > 1:
            if not server_info[1].isdigit():
                raise UsageError(
                    "Invalid format for --server parameter. Use <host>:<port>"
                )
        ...
        server_module = importlib.import_module(
            f"nominatim_api.server.{args.engine}.server"
        )
