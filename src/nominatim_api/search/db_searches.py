...
def filter_by_area(
    sql: SaSelect, t: SaFromClause, details: SearchDetails, avoid_index: bool = False
) -> SaSelect:
    ...
    if details.viewbox is not None and details.bounded_viewbox:
        sql = sql.where(
            t.c.geometry.intersects(
                VIEWBOX_PARAM, use_index=not avoid_index and details.viewbox.area < 0.2
            )
        )
...
def _add_geometry_columns(
    sql: SaLambdaSelect, col: SaColumn, details: SearchDetails
) -> SaSelect:
    out = []
...
def _make_interpolation_subquery(
    table: SaFromClause,
    inner: SaFromClause,
    numerals: List[int],
    ...
    if len(numerals) == 1:
        sql = sql.where(
            sa.between(numerals[0], table.c.startnumber, table.c.endnumber)
        ...
        sql = sql.where(
            sa.or_(
                *(
                    sa.and_(
                        sa.between(n, table.c.startnumber, table.c.endnumber),
...
def _filter_by_layer(table: SaFromClause, layers: DataLayer) -> SaColumn:
    ...
    if layers & DataLayer.ADDRESS and layers & DataLayer.POI:
        ...
        orexpr.append(
            sa.and_(
                no_index(table.c.rank_address) == 30,
    ...
    if layers & DataLayer.MANMADE:
        ...
        orexpr.append(
            sa.and_(
                table.c.class_.not_in(tuple(exclude)),
        ...
        orexpr.append(
            sa.and_(
                table.c.class_.in_(tuple(include)), no_index(table.c.rank_address) == 0
            )
        )
...
def _interpolated_position(table: SaFromClause, nr: SaColumn) -> SaColumn:
    pos = sa.cast(nr - table.c.startnumber, sa.Float) / (
        table.c.endnumber - table.c.startnumber
    )
    return sa.case(
        (table.c.endnumber == table.c.startnumber, table.c.linegeo.ST_Centroid()),
        else_=table.c.linegeo.ST_LineInterpolatePoint(pos),
    ).label("centroid")


async def _get_placex_housenumbers(
    conn: SearchConnection, place_ids: List[int], details: SearchDetails
) -> AsyncIterator[nres.SearchResult]:
    t = conn.t.placex
    sql = (
        _select_placex(t).add_columns(t.c.importance).where(t.c.place_id.in_(place_ids))
    )
...
def _int_list_to_subquery(inp: List[int]) -> "sa.Subquery":
    ...
    """
    vtab = sa.func.JsonArrayEach(sa.type_coerce(inp, sa.JSON)).table_valued(
        sa.column("value", type_=sa.JSON)
    )
    return sa.select(
        sa.cast(sa.cast(vtab.c.value, sa.Text), sa.Integer).label("nr")
    ).subquery()


async def _get_osmline(
...
class AbstractSearch(abc.ABC):
    ...
    @abc.abstractmethod
    async def lookup(
        self, conn: SearchConnection, details: SearchDetails
    ) -> nres.SearchResults:
...
class NearSearch(AbstractSearch):
    ...
    def __init__(
        self, penalty: float, categories: WeightedCategories, search: AbstractSearch
    ) -> None:
    ...
    async def lookup(
        ...
        if base[0].rank_address == 0:
            ...
            min_rank = 26
            max_rank = 30
        base = nres.SearchResults(
            r
            for r in base
            if (
                r.source_table == nres.SourceTable.PLACEX
        ...
        if base:
            ...
            for category, penalty in self.categories:
                await self.lookup_category(
                    results, conn, baseids, category, penalty, details
                )
    ...
    async def lookup_category(
        ...
        if table is None:
            ...
            sql = (
                sa.select(
                    table.c.place_id,
                    sa.func.min(tgeom.c.centroid.ST_Distance(table.c.centroid)).label(
                        "dist"
                    ),
                )
                .join(
                    tgeom, table.c.geometry.intersects(tgeom.c.centroid.ST_Expand(0.01))
                )
            ...
            sql = sa.select(
                ...
                table.c.centroid.ST_CoveredBy(
                    sa.case(
                        (
                            sa.and_(
                                tgeom.c.rank_address > 9, tgeom.c.geometry.is_area()
                            ),
                            tgeom.c.geometry,
                        ),
        ...
        inner = (
            sql.where(tgeom.c.place_id.in_(ids)).group_by(table.c.place_id).subquery()
        )

        t = conn.t.placex
        sql = (
            _select_placex(t)
            .add_columns((-inner.c.dist).label("importance"))
            .join(inner, inner.c.place_id == t.c.place_id)
            .order_by(inner.c.dist)
        )

        sql = sql.where(
            no_index(t.c.rank_address).between(MIN_RANK_PARAM, MAX_RANK_PARAM)
        )
...
class PoiSearch(AbstractSearch):
    ...
    async def lookup(
        ...
        if (
            details.near
            and details.near_radius is not None
            and details.near_radius < 0.2
        ):
            ...
            def _base_query() -> SaSelect:
                return (
                    _select_placex(t)
                    .add_columns(
                        (-t.c.centroid.ST_Distance(NEAR_PARAM)).label("importance")
                    )
                    .where(t.c.linked_place_id is None)
            ...
            if len(classtype) == 1:
                ...
                sql = _base_query().where(
                    sa.or_(
                        *(
                            sa.and_(t.c.class_ == cls, t.c.type == typ)
            ...
            for category in self.qualifiers.values:
                ...
                if table is not None:
                    ...
                    if details.near and details.near_radius is not None:
                        sql = sql.order_by(
                            table.c.centroid.ST_Distance(NEAR_PARAM)
                        ).where(
                            table.c.centroid.within_distance(
                                NEAR_PARAM, NEAR_RADIUS_PARAM
                            )
                        )
        ...
        for row in rows:
            result = nres.create_from_placex_row(row, nres.SearchResult)
            assert result
            result.accuracy = self.penalty + self.qualifiers.get_penalty(
                (row.class_, row.type)
            )
            result.bbox = Bbox.from_wkb(row.bbox)
            results.append(result)
...
class CountrySearch(AbstractSearch):
    ...
    async def lookup(
        ...
        for row in await conn.execute(sql, _details_to_bind_params(details)):
            result = nres.create_from_placex_row(row, nres.SearchResult)
            assert result
            result.accuracy = self.penalty + self.countries.get_penalty(
                row.country_code, 5.0
            )
            result.bbox = Bbox.from_wkb(row.bbox)
            results.append(result)
    ...
    async def lookup_in_country_table(
        self, conn: SearchConnection, details: SearchDetails
    ) -> nres.SearchResults:
        ...
        sql = (
            sa.select(
                tgrid.c.country_code,
                tgrid.c.geometry.ST_Centroid()
                .ST_Collect()
                .ST_Centroid()
                .label("centroid"),
...
class PostcodeSearch(AbstractSearch):
    ...
    async def lookup(
        ...
        if details.viewbox is not None and not details.bounded_viewbox:
            penalty += sa.case(
                (t.c.geometry.intersects(VIEWBOX_PARAM), 0.0),
        ...
        if self.lookups:
            ...
            sql = sql.where(tsearch.c.place_id == t.c.parent_place_id).where(
                (tsearch.c.name_vector + tsearch.c.nameaddress_vector).contains(
                    sa.type_coerce(self.lookups[0].tokens, IntArray)
                )
            )
        ...
        penalty += sa.case(
            *((t.c.postcode == v, p) for v, p in self.postcodes), else_=1.0
        )
        ...
        for row in await conn.execute(sql, _details_to_bind_params(details)):
            ...
            for prow in await conn.execute(
                placex_sql, _details_to_bind_params(details)
            ):
...
class PlaceSearch(AbstractSearch):
    ...
    def __init__(
        self, extra_penalty: float, sdata: SearchData, expected_count: int
    ) -> None:
        super().__init__(sdata.penalty + extra_penalty)
        self.countries = sdata.countries
        self.postcodes = sdata.postcodes
    ...
    def _inner_search_name_cte(
        ...
        sql = sa.select(
            ...
            t.c.centroid,
            t.c.name_vector,
            t.c.nameaddress_vector,
            sa.case(
                (t.c.importance > 0, t.c.importance),
        ...
        if details.viewbox is not None:
            if details.bounded_viewbox:
                sql = sql.where(
                    t.c.centroid.intersects(
                        VIEWBOX_PARAM, use_index=details.viewbox.area < 0.2
                    )
                )
            elif (
                not self.postcodes
                and not self.housenumbers
                and self.expected_count >= 10000
            ):
                sql = sql.where(
                    t.c.centroid.intersects(
                        VIEWBOX2_PARAM, use_index=details.viewbox.area < 0.5
                    )
                )
        ...
        if details.near is not None and details.near_radius is not None:
            if details.near_radius < 0.1:
                sql = sql.where(
                    t.c.centroid.within_distance(NEAR_PARAM, NEAR_RADIUS_PARAM)
                )
            else:
                sql = sql.where(
                    t.c.centroid.ST_Distance(NEAR_PARAM) < NEAR_RADIUS_PARAM
                )
        ...
        if self.housenumbers:
            ...
            if details.min_rank > 0:
                sql = sql.where(
                    sa.or_(
                        t.c.address_rank >= MIN_RANK_PARAM,
            ...
            if details.max_rank < 30:
                sql = sql.where(
                    sa.or_(
                        t.c.address_rank <= MAX_RANK_PARAM,
        ...
        sql = sa.select(
            inner.c.place_id,
            inner.c.search_rank,
            inner.c.address_rank,
            inner.c.country_code,
            inner.c.centroid,
            inner.c.importance,
            inner.c.penalty,
        )
        ...
        if (
            ...
            sql = sql.add_columns(
                sa.func.first_value(inner.c.penalty - inner.c.importance)
    ...
    async def lookup(
        ...
        if self.postcodes:
            ...
            pc_near = (
                sa.select(sa.func.min(tpc.c.geometry.ST_Distance(t.c.centroid)))
                .where(tpc.c.postcode.in_(pcs))
                .scalar_subquery()
            )
            penalty += sa.case(
                (t.c.postcode.in_(pcs), 0.0),
                else_=sa.func.coalesce(pc_near, cast(SaColumn, 2.0)),
            )

        if details.viewbox is not None and not details.bounded_viewbox:
            penalty += sa.case(
                (t.c.geometry.intersects(VIEWBOX_PARAM, use_index=False), 0.0),
        ...
        if details.near is not None:
            sql = sql.add_columns(
                (-tsearch.c.centroid.ST_Distance(NEAR_PARAM)).label("importance")
            )
        ...
        sql = sql.add_columns(penalty.label("accuracy")).order_by(sa.text("accuracy"))

        if self.housenumbers:
            hnr_list = "|".join(self.housenumbers.values)
            inner = sql.where(
                sa.or_(
                    tsearch.c.address_rank < 30,
            ...
            place_sql = (
                ...
                .where(thnr.c.linked_place_id is None)
                .where(thnr.c.indexed_status == 0)
            )

            if details.excluded:
                place_sql = place_sql.where(
            ...
            numerals = [
                int(n) for n in self.housenumbers.values if n.isdigit() and len(n) < 8
            ]
            ...
            if numerals and (
                ...
                interpol_sql = _make_interpolation_subquery(
                    conn.t.osmline, inner, numerals, details
                )
                # Housenumbers from Tiger
                tiger_sql = sa.case(
                    (
                        inner.c.country_code == "us",
                        _make_interpolation_subquery(
                            conn.t.tiger, inner, numerals, details
                        ),
                    ),
                    else_=None,
                )
            ...
            unsort = sa.select(
                inner,
                place_sql.scalar_subquery().label("placex_hnr"),
                interpol_sql.label("interpol_hnr"),
                tiger_sql.label("tiger_hnr"),
            ).subquery("unsort")
            sql = sa.select(unsort).order_by(
                sa.case(
                    (unsort.c.placex_hnr is not None, 1),
                    (unsort.c.interpol_hnr is not None, 2),
                    (unsort.c.tiger_hnr is not None, 3),
                    else_=4,
                ),
                unsort.c.accuracy,
            )
        else:
            sql = sql.where(t.c.linked_place_id is None).where(t.c.indexed_status == 0)
            if self.qualifiers:
        ...
        for row in await conn.execute(sql, _details_to_bind_params(details)):
            ...
            if self.housenumbers and row.rank_address < 30:
                if row.placex_hnr:
                    ...
                    subs = _get_tiger(
                        conn, row.tiger_hnr, numerals, row.osm_id, details
                    )
                else:
                    subs = None
                ...
                if subs is not None:
                    async for sub in subs:
                        ...
                        if not any(
                            nr in self.housenumbers.values
                ...
                # filter conditions.
                if (
                    (not details.excluded or result.place_id not in details.excluded)
                    and (
                        not self.qualifiers or result.category in self.qualifiers.values
                    )
